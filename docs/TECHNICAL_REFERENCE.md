...
|------|----------|---------|-------------|
| `--dry-run` | None | false | Simulate updates without making DNS changes. Logs intended actions with "[DRY-RUN]" prefix. Notifications are still delivered so the channel configuration can be verified end to end, but the cooldown is not consumed, so a dry-run cannot suppress a later real alert. |
